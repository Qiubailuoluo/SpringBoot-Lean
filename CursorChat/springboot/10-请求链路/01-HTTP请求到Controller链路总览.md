...
## 代码位置
- 启动：`com/bookshop/BookshopApplication.java`
- Trace 过滤器：`common/trace/TraceIdFilter.java`
- 安全链：`config/security/SecurityConfig.java`
- JWT 过滤器：`config/security/JwtAuthenticationFilter.java`

## JavaWeb 与 SpringBoot 关系（简版）
- **JavaWeb**   提供基础能力（按照流程顺序）：
	- Tomcat 容器
	- Filter 过滤器
	- Servlet 规范
- **Spring MVC**   构建在 Servlet 之上：
	- `DispatcherServlet` 负责请求分发到 Controller。
- **Spring Boot** ：
	- 负责“自动装配 + 嵌入式 Tomcat + 约定配置”，
	- 把 JavaWeb 与 SpringMVC 快速整合成可运行应用。
	- Springboot = [JavaWeb + [Spring MVC] ]

## 执行链路
1. 嵌入式 Tomcat 接收 HTTP 请求（`spring-boot-starter-web` 默认行为）。
2. `TraceIdFilter` 最先执行，生成 `traceId`，写入 MDC、request attribute、`X-Trace-Id` 响应头。   -->(Trace 是请求链路追踪体系)
3. Spring Security FilterChain 执行，按白名单或鉴权策略处理。  -->(Security 安全认证体系)
...
- `com/bookshop/config/security/JwtAuthenticationFilter.java`
